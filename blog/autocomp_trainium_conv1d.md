...
It isn’t entirely clear why Autocomp chose a tile size of 64 instead of the more intuitive 128. However, we find that regardless of the value chosen, tiling by values near 128 (such as 32 or 64) appears enough for the compiler to perform its own grouping. The small performance deltas (around 1%) and the operation counts we show next support this idea.
